...
IfcOWL provides a Web Ontology Language (OWL) representation of the Industry Foundation Classes (IFC) schema. IfcOWL was an academic experiment that concluded that IFC is difficult to represent in formats other than EXPRESS. The IfcOWL is full of exceptions and particularities that make it hard to use in practice.

Semantic web enthusiasts are advised to use the [bSDD](http://bsdd.buildingsmart.org/) that serves TTL for every URI and a GraphQL interface.
